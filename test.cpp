/*
    ...
    目前进度（11.30 16:32）：
        目前mac环境下能够实现在win虚拟机下测试，因此需要回到之前的问题：
        C1的每次生成都受随机数k的控制，因此如何确定k的取值是目前需要研究的问题。
        目前推测是随机数的大小限制(0,n-1]存在一定的可修改的地方
        主要原因在于在vli_cmp的内部比较是倒序比较的，但是为什么设计成这个样子至今仍然无法解释
        在很多计算的步骤中基本上都是事先将计算的值倒序，然后再进行计算的操作
*/
